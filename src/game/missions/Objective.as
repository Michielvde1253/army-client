	...
	public class Objective implements IconInterface {
		...
		public function initialize(param1: String, param2: Object): void {
			...
			if ((param1 == Mission.TYPE_MODAL_ARROW || param1 == Mission.TYPE_INTEL) && this.mParameter is Array) {
				...
				if (this.mId == "T_1" || this.mId == "T_2" || this.mId == "T_3" || this.mId == "T_5" || this.mId == "T_7" || this.mId == "T_8") {
					// Tutorial: move camera
					var defaultX:int = this.mParameter[0];
					var defaultY:int = this.mParameter[1];
					var centerPointX:Number = GameState.mInstance.mScene.getCenterPointXAtIJ(defaultX, defaultY);
					var centerPointY:Number = GameState.mInstance.mScene.getCenterPointYAtIJ(defaultX, defaultY);
					GameState.mInstance.mScene.mCamera.setTargetTo(centerPointX, centerPointY);
				}
			}
		}
