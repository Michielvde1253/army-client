package game.actions
{
   ...
   public class RepairDecorationAction extends Action
   {
      ...
      public function RepairDecorationAction(param1:DecorationObject)
      {
         super("RepairDecorationAction");
         mTarget = param1;
         this.mTimer = 0;
         this.mState = -1;
         this.mNewState = -1;
      ...
      override public function start() : void
      {
         ...
         var _loc3_:DecorationObject = mTarget as DecorationObject;
         ...
         mTarget.playSound(ArmySoundManager.SFX_UI_REPAIR);
      }
      
      private function isLegal() : Boolean
      {
         return Boolean(mTarget) && !(mTarget as DecorationObject).isFullHealth();
      }
      ...
      private function execute() : void
      {
         if(!mTarget)
         {
            Utils.LogError("RepairDecoration: Decoration not found");
            this.mNewState = STATE_OVER;
            return;
         }
         if(this.mState != STATE_REPAIRING)
         {
            Utils.LogError("RepairDecoration: Illegal state");
            this.mNewState = STATE_OVER;
            return;
         }
         ...
         var _loc5_:DecorationItem;
         var _loc4_:DecorationObject;
         var _loc6_:int = (_loc5_ = (_loc4_ = mTarget as DecorationObject).mItem as DecorationItem).mHealRewardXP;
         ...
         GameState.mInstance.mServer.serverCallServiceWithParameters(ServiceIDs.REPAIR_PLAYER_DECORATION,_loc12_,false);
