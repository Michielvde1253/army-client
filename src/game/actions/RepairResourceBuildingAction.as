package game.actions
{
   ...
   public class RepairResourceBuildingAction extends Action
   {
      ...
      public function RepairResourceBuildingAction(param1:ResourceBuildingObject)
      {
         super("RepairResourceBuildingAction");
         mTarget = param1;
         this.mTimer = 0;
         this.mState = -1;
         this.mNewState = -1;
      ...
      private function changeState(param1:int) : void
      {
         ...
         {
            case STATE_REPAIRING:
               (mTarget as ResourceBuildingObject).startRepairing();
               break;
            case STATE_OVER:
               (mTarget as ResourceBuildingObject).hideLoadingBar();
         }
      }
      ...
      override public function start() : void
      {
         ...
         var _loc3_:ResourceBuildingObject = mTarget as ResourceBuildingObject;
      ...
      {
         if((mTarget as ResourceBuildingObject).getState() == PlayerBuildingObject.STATE_RUINS)
         {
            return (mTarget as ResourceBuildingObject).isRepairable();
         }
         if(!(mTarget as ResourceBuildingObject).isFullHealth())
         {
            return true;
         }
         return false;
      ...
      private function execute() : void
      {
         if(!mTarget)
         {
            Utils.LogError("RepairResourceBuilding: Building not found");
            this.mNewState = STATE_OVER;
            return;
         }
         if(this.mState != STATE_REPAIRING)
         {
            Utils.LogError("RepairResourceBuilding: Illegal state");
            this.mNewState = STATE_OVER;
            return;
         }
         ...
         var _loc5_:ResourceBuildingItem;
         var _loc4_:ResourceBuildingObject;
         var _loc6_:int = (_loc5_ = (_loc4_ = mTarget as ResourceBuildingObject).mItem as ResourceBuildingItem).mHealRewardXP;
