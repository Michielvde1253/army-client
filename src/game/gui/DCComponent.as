package game.gui
{
   ...
   public class DCComponent extends MovieClip
   {
      ...
      public static const MODAL_ALPHA_RECTANGLE_COLOR:uint = 0;
      ...
      protected function setModal() : void
      {
         ...
         if(_loc1_ == null)
         {
            _loc2_ = Math.min(this.mParent.width,this.mParent.width);
            _loc3_ = Math.min(this.mParent.height,this.mParent.height);
            ...
            this.mCustomModalSquare.graphics.drawRect(0,0,_loc2_,_loc3_);
            ...
            this.mCustomModalSquare.addEventListener(MouseEvent.MOUSE_DOWN,this.mouseHandler,false,0,true);
            this.mCustomModalSquare.addEventListener(MouseEvent.MOUSE_UP,this.mouseHandler,false,0,true);
            this.mCustomModalSquare.addEventListener(MouseEvent.CLICK,this.mouseHandler,false,0,true);
      ...
      protected function removeModalSquare() : void
      {
         ...
         if(this.isModal())
         {
            ...
            if(_loc1_)
            {
               ...
               {
                  _loc2_ = false;
                  _loc3_ = this.mParent.getChildIndex(_loc1_);
                  _loc4_ = this.mParent.numChildren - 1;
                  while(_loc4_ >= 0)
                  {
                     if((_loc5_ = this.mParent.getChildAt(_loc4_)) is PopUpWindow && _loc5_ != this)
                     {
                        if(_loc4_ > 0 && !(this.mParent.getChildAt(_loc4_ - 1) is PopUpWindow) && this.mParent.getChildAt(_loc4_ - 1) != _loc1_)
                        {
                           _loc4_--;
                        }
                        if(_loc3_ < _loc4_)
                        {
                           this.mParent.setChildIndex(_loc1_,_loc4_ - 1);
                        }
                        else
                        {
                           this.mParent.setChildIndex(_loc1_,_loc4_);
                        }
                        _loc2_ = true;
                        break;
                     }
                  ...
                  if(!_loc2_)
                  {
                     Utils.LogError("Could not move modal square because lower menu was not found");
                     this.mParent.removeChild(_loc1_);
                     _loc1_ = null;
                  }
               }
      ...
      public function open(param1:DisplayObjectContainer, param2:Boolean = false) : void
      {
         ...
         com.dchoc.utils.CursorManager.getInstance().bringToFront();
      ...
      public function enterFrame(param1:Event) : void
      {
         ...
         {
            case STATE_OPENING:
               if(this.mOpenAnimation.getCurrentLabel() == COMPONENT_FRAME_NAME_NORMAL)
               {
